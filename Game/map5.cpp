...
int map5::mapDraw(int* x, int* y, int *c, int *d) {
	system("CLS");
	hero character;
	int k, a = 1, b = 17, l, flag = 0, enemy=50;
	int torpedo = 15;
	bossX = new int[15];
	bossY = new int[15];
	for (k = 0; k < 15; k++) {
	...
	locate(x[45], y[45]); std::cout << "$";
	locate(x[46], y[46]); std::cout << "$";
	locate(x[47], y[47]); std::cout << "$";
	locate(x[48], y[48]); std::cout << "$";
	locate(x[49], y[49]); std::cout << "$";
	setColor(LIGHTRED);
